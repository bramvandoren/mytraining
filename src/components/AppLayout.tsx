...
const NAV = [
  ...
  { to: "/season", label: "Season", icon: CalendarRange },
  { to: "/community", label: "Community", icon: Globe },
  { to: "/club", label: "Club", icon: Building2 },
];

const MOBILE_BOTTOM = NAV.slice(0, 4); // Dashboard, Library, Sessions, Templates
...
export default function AppLayout() {
  ...
  // Listen for layout-level events from pages (e.g. dashboard ?open shortcuts)
  useEffect(() => {
  ...
  const Sidebar = (
    <nav className="flex flex-col h-full bg-card">
      <div className="px-4 h-14 flex items-center gap-2 border-b border-border">
        ...
        <span className="font-semibold tracking-tight">Training Architect</span>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-0.5">
        ...
        >
          ...
          <span className="flex-1 text-left">Session Builder</span>
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between p-3 border-b border-border h-14 flex-shrink-0">
                  <span className="text-sm font-semibold">Session Builder</span>
                  <button
                    onClick={() => setBuilderOpen(false)}
